...
var specialCategories = ['Require', 'Import', 'Methods', 'Properties']
var token = '@@token@@'
...
function generateDoc(source, options) {
    ...
    _.each(api, function(entry) {
        ...
        if (byCategories) {
            ...
            if (
                !member ||
                entry.isCtor() ||
                (entry.getType() == 'Object' && !/[=:]\s*(?:null|undefined)\s*[,;]?$/gi.test(entry.entry))
            ) {
        ...
        _.each(['entryHref', 'sourceHref', 'tocHref', 'entryLink', 'sourceLink', 'tocLink'], function(option) {
        ...
        if (!_.isEmpty(relatedItems)) {
            entryMarkdown.push('#### Related', relatedItems.join(', '), '')
        }
    ...
    // Add TOC headers.
    // log.lightMagenta('organized', organized)

    var tocGroups = ["Import", "Require", "Date", "Math",]

    log.lightYellow('tocGroups', tocGroups)
    if (byCategories) {
        // Remove special categories before sorting.
        var catogoriesUsed = _.intersection(specialCategories, tocGroups)
        _.pullAll(catogoriesUsed, tocGroups)
    ...
    _.each(tocGroups, function(group) {
        tocMarkdown.push('<!-- div class="import" -->\n', '## `' + group + '`')
        ...
        _.each(organized[group], function(entry) {
            ...
            if (entry.isAlias()) {
                ...
                tocMarkdown.push('* ' + makeAnchor('#' + entry.getHash(style), '`' + title + '`'))
            }
        })
        tocMarkdown.push('\n<!-- /div -->\n')
    })
